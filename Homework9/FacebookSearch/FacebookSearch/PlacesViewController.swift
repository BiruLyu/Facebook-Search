...
class PlacesViewController: UIViewController, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let url = "https://helloworld-163003.appspot.com/?Places="+keyword
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let pictureURL = URL(string: self.placeList[indexPath.row].getImage())!
        let pictureData = NSData(contentsOf: pictureURL as URL)
        let catPicture = UIImage(data: pictureData as! Data)
        
        
        cell.photo.image = catPicture;
        cell.name.text = self.placeList[indexPath.row].getName()
        
        if self.placeList[indexPath.row].getFavorite() {
